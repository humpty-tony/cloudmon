...
func (s *Store) Evidence(seq int64, offset int) (EvidencePage, error) {
	...
	if err := s.queryJSON("SELECT count(*) AS total,count(DISTINCT o.sha256) AS variants"+where+";", &totals); err != nil {
		return result, err
	}
	...
	err := s.queryJSON("SELECT o.id,o.sha256,o.source,o.ordinal,o.format,o.lossy,o.observedAt,o.sha256=e.evidenceHash AS displayed"+where+fmt.Sprintf(" ORDER BY o.id LIMIT 25 OFFSET %d;", offset), &result.Observations)
	return result, err
}
