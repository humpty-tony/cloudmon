...
function HuntResultInspector({row,snapshot,onClose,...props}:Pick<EventInspectorProps,"onPivot"|"timeZone"|"onClose"> & {row:EventRow;snapshot:EvidenceSnapshot}) {
  ...
  return <><EventInspector {...props} event={event} snapshot={snapshot} rawJSON={raw} rawError={rawError} lineage={lineage} lineageError={lineageError} onRetry={()=>setRetry(value=>value+1)} onOpenLineage={()=>setGraph(true)} onClose={onClose}/>{graph&&<LineageView seq={row.seq} initialSnapshot={snapshot} onClose={()=>setGraph(false)} onPivot={props.onPivot}/>}</>;
}
...
export function HuntView({filter,fixedMode,savedLoad,onLoadSaved,inspector}:HuntViewProps) {
  ...
  return <div className="analysis-view hunt-view">
    ...
    <div className="analysis-scope">{useFilter?`Scope: ${fixedMode?"current Workbench":"console"} filters, including time bounds, apply to every step.`:savedFilter?"Scope: saved filters. Filter values and absolute time bounds are fixed until you change scope.":"Scope: all loaded evidence."} Runs on request. Matches are investigative context.</div>
    {savedFilter&&!useFilter&&<div className="saved-hunt-scope"><details><summary>Saved filter scope</summary><pre>{JSON.stringify(savedFilter,null,2)}</pre></details><div className="saved-hunt-actions"><button className="btn-ghost btn-sm" disabled={busy} onClick={()=>{setSavedFilter(null);setUseFilter(true)}}>Use current {fixedMode?"Workbench":"console"} filters</button><button className="btn-ghost btn-sm" disabled={busy} onClick={()=>{setSavedFilter(null);setUseFilter(false)}}>Clear saved scope</button></div></div>}
