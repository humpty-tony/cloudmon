...
const INDICATOR_EXAMPLES:Record<HuntIndicator["kind"],string>={ip:"192.0.2.1",cidr:"2001:db8::/32",key:"ASIAEXAMPLE",event:"event-id",arn:"arn:aws:s3:::example-bucket"};
...
export function HuntView({filter,fixedMode,savedLoad,onLoadSaved,inspector}:HuntViewProps) {
  ...
  const bulkId=useId();
  const indicatorLines=useMemo(()=>text.split(/\r?\n/).map((line,index)=>({line,index})).filter(item=>item.line.trim()),[text]);
  const example=`${indicatorType} ${INDICATOR_EXAMPLES[indicatorType]}`;
  const removeIndicator=(index:number)=>setText(current=>current.split(/\r?\n/).filter((_,i)=>i!==index).join("\n"));
  const appendIndicator=(line:string)=>setText(current=>`${current}${current&&!current.endsWith("\n")?"\n":""}${line}`);
  const canAddIndicator=indicatorLines.length<100&&!!indicatorValue.trim()&&!/[\r\n]/.test(indicatorValue)&&text.length+indicatorValue.trim().length+indicatorType.length+2<=256000;
  ...
  return <div className="analysis-view hunt-view">
    <div className="hunt-commandbar" role="group" aria-label="Hunt controls">
    <div className="hunt-scope-controls">{!fixedMode?<><label>Hunt type <select aria-label="Hunt type" value={mode} disabled={busy} onChange={e=>setMode(e.target.value as HuntOptions["mode"])}><option value="indicators">Bulk indicators</option><option value="sequence">Ordered events (2–5 steps)</option></select></label><label><input type="checkbox" checked={useFilter} disabled={busy} onChange={e=>{setUseFilter(e.target.checked);setSavedFilter(null)}}/> Use console filters</label></>:<label>Scope <select aria-label="Hunt scope" value={useFilter?"workbench":savedFilter?"saved":"all"} disabled={busy} onChange={e=>{setUseFilter(e.target.value==="workbench");if(e.target.value!=="saved")setSavedFilter(null)}}><option value="all">All loaded evidence</option><option value="workbench">Current event filters</option>{savedFilter&&<option value="saved">Saved filters (fixed)</option>}</select></label>}</div>
      <div className="hunt-run-actions"><button className="btn-primary" disabled={busy||!!validation} onClick={()=>void run()}>Run hunt</button>{busy&&<button className="btn-ghost btn-sm" onClick={()=>controller.current?.abort()}>Cancel hunt</button>}</div>
    </div>
    <div className="hunt-author">
    <header className="analysis-heading"><h1>{mode==="indicators"?"Indicators to match":"Sequence steps"}</h1></header>
    {mode==="indicators"?<>
      <button className="hunt-bulk-toggle" aria-expanded={showBulk} aria-controls={bulkId} disabled={busy} onClick={()=>setShowBulk(value=>!value)}>{showBulk?"Back to indicator list":"Paste multiple indicators"}</button>
      <div hidden={showBulk}>
        <div className="hunt-indicator-add">
          ...
          <label>Value <input aria-label="Indicator value" value={indicatorValue} disabled={busy} maxLength={16384} onChange={e=>setIndicatorValue(e.target.value)} placeholder={INDICATOR_EXAMPLES[indicatorType]}/></label>
        ...
        <div className="hunt-input-example"><code>{example}</code><button aria-label="Add example indicator" disabled={busy||indicatorLines.length>=100||text.length+example.length+1>256000} onClick={()=>appendIndicator(example)}>Use example</button></div>
        <section className="hunt-indicator-list" aria-label="Indicators to match">
          <div className="hunt-indicator-count">{indicatorLines.length} / 100 indicators</div>
          {indicatorLines.length===0?<p>No indicators added.</p>:indicatorLines.slice(0,100).map(({line,index})=><div className="hunt-indicator-item" key={index}><code title={line}>{line}</code><button disabled={busy} aria-label={`Remove indicator ${index+1}`} onClick={()=>removeIndicator(index)}>×</button></div>)}
          {indicatorLines.length>100&&<p>Too many indicators. Edit the bulk input to reduce the list.</p>}
        </section>
      </div>
      <label className="hunt-input" id={bulkId} hidden={!showBulk}>Typed indicators <textarea aria-label="Typed indicators" value={text} disabled={busy} maxLength={256000} onChange={e=>setText(e.target.value)} spellCheck={false} placeholder={'ip 192.0.2.1\ncidr 2001:db8::/32\nkey ASIAEXAMPLE\nevent event-id\narn arn:aws:s3:::example-bucket'}/><span>One type and value per line · up to 100. IDs and ARNs match exactly.</span></label></>:<div className="hunt-sequence-controls">
      <details className="hunt-sequence-starter"><summary>Example sequence</summary><button className="btn-ghost btn-sm" disabled={busy||steps.some(step=>!!step.trim())} onClick={()=>{setSteps(['eventName="ConsoleLogin"','eventName="GetObject"']);setGroup("principal");setMinutes(15)}}>Load sequence example</button><span>ConsoleLogin → GetObject · same principal ARN · within 15 minutes. Fills empty steps only; does not run.</span></details>
      ...
      <details className="hunt-sequence-rules"><summary>How matching works</summary><p>Each step must be strictly later than the previous step. At each stage, select the closest completed prefix. The complete first-to-last interval must fit the selected window. Each final event has one representative sequence.</p></details>
    </div>}
    ...
    {validation&&(mode==="indicators"?!!text.trim():steps.some(step=>!!step.trim()))&&<div className="hunt-validation" role="status">{validation}</div>}
    </div>
    ...
    {!result&&!busy&&!error&&<div className="analysis-empty">Run the hunt to view {mode==="indicators"?"matching events":"matching sequences"}.</div>}
    </div>
