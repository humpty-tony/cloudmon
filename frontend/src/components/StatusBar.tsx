...
export function StatusBar(p: Props) {
  ...
  return (
    <div className="statusbar">
      ...
      <span className="sb-cell" style={p.source ? { color: "var(--acc-text)" } : undefined}>
        {!p.streaming ? "imported file" : p.source ? p.source : p.live ? "sqs" : "mock feed"}
      </span>
      ...
      <span className="sb-center">
        {p.streaming && p.loading ? (
          ...
          <button className="newpill" onClick={p.onRepin}>
            ↑ {p.newCount.toLocaleString()} new events
          </button>
        ) : null}
      </span>
      ...
      <span className="sb-hint">
        <kbd>j</kbd>/<kbd>k</kbd> move · <kbd>enter</kbd> open · <kbd>/</kbd> filter · <kbd>f</kbd> pivot · <kbd>Ctrl K</kbd> cmds
      </span>
