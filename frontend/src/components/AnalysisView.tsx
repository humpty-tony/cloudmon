...
export function AnalysisView({filter}:{filter:QueryFilter}) {
  ...
  return <main className="analysis-view">
    ...
    {busy?<div className="analysis-empty" role="status">Reading activity snapshot…</div>:error?<div className="analysis-empty" role="alert">{error}</div>:!result?<div className="analysis-empty">Run analysis to rank activity. Select a result to inspect its services, operations, source addresses and original records.</div>:applied&&<>
      ...
      <div className="analysis-table-wrap"><table className="analysis-table"><thead><tr><th>{label}</th><th>{applied.compare?"Current":"Events"}</th>{applied.compare&&<><th>Previous</th><th>Change</th></>}<th>Errors</th><th>Writes</th></tr></thead><tbody>{result.groups.map(group=><tr key={group.value}><td><button className="analysis-entity" disabled={busy} onClick={()=>void run({...applied,entity:{dimension:applied.dimension,value:group.value},snapshot:result.snapshot})}>{group.value||"Not recorded"}<AliasBadge field={applied.dimension} value={group.value}/></button></td><td>{number(group.current)}</td>{applied.compare&&<><td>{number(group.previous)}</td><td className={group.current>group.previous?"analysis-increase":""}>{group.current>group.previous?"+":""}{number(group.current-group.previous)}{group.previous===0&&group.current>0&&<small>New in compared window</small>}</td></>}<td>{number(group.errors)}</td><td>{number(group.writes)}</td></tr>)}</tbody></table>{!result.groups.length&&<p>No events match this scope.</p>}</div>
      ...
      {applied.entity&&<><div className="analysis-breakdowns">{Object.entries(result.breakdowns).map(([field,groups])=><section key={field}><h3>{analysisDimensions.find(([key])=>key===field)?.[1]}</h3>{groups.map(g=><div key={g.value}><span>{g.value||"Not recorded"}<AliasBadge field={field} value={g.value}/></span><b>{number(g.current)}</b></div>)}<small>Top 10 · current counts</small></section>)}</div>
