...
function SelectedInspector({ event: e, layout = "side", snapshot, rawJSON, rawLoading, rawError, lineage, lineageLoading, lineageError, onRetry, onPivot, onOpenLineage, onInvestigate, onClose, timeZone, tab, onSelectTab }: EventInspectorProps & { event: CloudTrailEvent; tab: Tab; onSelectTab: (tab: Tab) => void }) {
  ...
  const items = dock ? DOCK_TABS : TABS;
  const [fieldsVisited, setFieldsVisited] = useState(!dock || tab === "fields");
  ...
  const actions = <div className="ei-actions" aria-label={`Selected event actions: ${e.eventName} · ${e.eventID}`}>
      <button className="ei-investigate" disabled={!onInvestigate && !snapshot} title={`Around selected event: ${e.eventName} · ${e.eventID}`} onClick={() => onInvestigate ? handOff(() => onInvestigate(e, snapshot)) : setDialog("investigate")}>{dock || onInvestigate ? "Around this event" : "Investigate"}</button>
  ...
  return <>
    <header className="ei-head">
      <div className="ei-heading"><span className="ei-eyebrow">{dock ? "Selected event" : "Event inspector"}</span><h2 title={`${e.eventName} · ${e.eventID}`}>{e.eventName}</h2></div>
      {dock && actions}
      <button className="ei-close" onClick={onClose} aria-label="Close inspector" title="Clear selected event">×</button>
    </header>
    {dock ? <div className="ei-dock-body">{context}<div className="ei-details">{details}</div></div> : <>{context}{actions}{details}</>}
...
export const EventInspector = memo(function EventInspector(props: EventInspectorProps) {
  ...
  const [mode, setMode] = useState<{layout: "dock" | "side"; tab: Tab}>(() => ({layout, tab: layout === "dock" ? "lineage" : "fields"}));
  const tab = mode.layout === layout ? mode.tab : layout === "dock" ? "lineage" : "fields";
