...
export interface HuntOptions { mode: "indicators" | "sequence"; filter: QueryFilter; indicators: HuntIndicator[]; first: QueryExpr | null; second: QueryExpr | null; steps?: QueryExpr[] | null; group: "principal" | "credential"; minutes: number; snapshot: EvidenceSnapshot | null }
...
export interface HuntSequence { events: EventRow[]; deltaMs: number; tiedCandidates: number[] }
export interface HuntResult { snapshot: EvidenceSnapshot; scanned: number; total: number; limit: number; invalidTimes: number; missingPrincipal: number; missingCredential: number; indicators: (HuntIndicator & {matches:number})[]; matches: HuntMatch[]; pairs: SequencePair[]; sequences: HuntSequence[]; notes: string[] }
