...
### Search action placement follow-up
...
- [x] Reproduced the query-planner weakness with failing tests for all six APIs, missing creation hints and ancestor hints. Added prioritized creation windows and fair EventName pagination for AssumeRole, SAML, OIDC, AssumeRoot, GetSessionToken and GetFederationToken under the existing shared bounds; added API-specific source coverage. Adapter/store suite, focused App cache/isolation tests and native build pass. Six-method fixtures include negative issuance tests; existing strict-store tests cover temporal/identity conflicts, cross-account evidence and cycles.
- [x] Live actual-App before/after comparisons: original S3 0→0 proven edges, fresh S3 0→0, service AssumeRole positive control 1→1. Fresh case scans 1,000→745 records with the SAME 20-call budget; no latency improvement claim. All six methods queried; SAML/OIDC/root/token pagination completes, broader AssumeRole remains partial. This improves search coverage, not proof that the SSO issuance gap is solved. Dataset and offline cache preserved; no capture/provisioning changes. Private reproducible overlays, comparisons and fresh screenshot: `~/.hermes/cache/scratch/cloudmon-lineage-audit/`.
- [x] Corrected the independent review's sole query bug: admit the selected seed hint before capped locals and preserve first execution. Separate fix worker reproduced RED with 12/13 local windows; 11/12/13-window regressions and attribution suite now pass, independently rerun by parent. Native binary rebuilt.
- [x] Final corrective review passed with no security or logic blockers: reserved seed admission/first execution and token display labels rechecked in a bounded independent review. Static review only; parent verified the regressions, real-AWS demos, browser evidence interactions, cleanup and native build. Implementation and checked handoff included in this commit.

### Authorized live relationship demos

- [x] Created narrowly scoped temporary IAM assets with explicit user authorization: two chained roles and one IAM user/access key, no compute/storage resources. Executed actual AssumeRole twice, GetSessionToken and GetFederationToken, followed by GetCallerIdentity with each relevant credential. Original IAM key disabled immediately after invocations; no SDK secret-access-key values persisted.
- [x] Independently located all three selected activity records and four successful issuance records in CloudTrail by actual request IDs, verifying returned/signer keys. Started each isolated App with only the selected activity, then resolved dynamically: role chain **2 AssumeRole edges**, temporary user **1 GetSessionToken edge**, federated session **1 GetFederationToken edge**. Actual-App test passed; dataset unchanged and offline cache readable for each case.
- [x] Actual browser UI replayed the unchanged live-native results. All four originals opened from their respective connections; browse return preserved. Three screenshots have account/email display redaction without changing native graph or raw records. Source identity → SSO role remains the recorded onBehalfOf association, not newly recovered SAML issuance.
- [x] Adversarial screenshot review caught duplicate/misleading token labels. A browser regression failed on all four expected label checks before the fix; it now passes with IAM USER versus TEMPORARY USER SESSION, and the FEDERATED SESSION name taken from its recorded ARN (`demo-federated`) rather than the issuing IAM user's name. Native build and diff checks pass.
- [x] Screenshots captured before cleanup. All demo roles, user, access key and inline policies deleted; direct GetRole/GetUser returned NoSuchEntity and run-path listings were empty. Rechecked all planned names across both rolled-back setup attempts and the successful run: nothing remains. Private manifests, raw evidence, overlays, cleanup readbacks and screenshots: `~/.hermes/cache/scratch/cloudmon-relationship-demo/`. Saved-evidence browser rendering is not native Wails-window acceptance.
- [x] Reproducible browser check: `CLOUDMON_DEMO_DIR=<private-demo-directory> node frontend/scripts/check-lineage-relationship-demo.mjs`. This requires real saved native reports and never provisions AWS or substitutes fabricated evidence.

### Graph-first identity → role → selected activity
